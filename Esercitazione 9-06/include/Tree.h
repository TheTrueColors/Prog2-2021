...
private:
    ...
    {
        if(tmp->getKey().getCavalli()== key.getCavalli())
        {
            return tmp;
        }
        else if(key.getCavalli()<tmp->getKey().getCavalli())
        {
    ...
    /** Metodo che ci restituisce il nodo passato come puntatore se la sua cilindrata è minore della cilindrata minima consentita*/
    Node<T> *_isMin(Node<T> *tmp,int key)
    {
        if(tmp->getKey().getCavalli()< key)
        {
            return tmp;
        }
    ...
    /** Metodo per la cancellazione ricorsiva dei veicoli che hanno una cilindrata sotto la cilindrata minima stabilita*/
    void _canc(Node<T> *current, int key)
    {
        if (current != NULL)
        {
            _canc(current->getLeft(),key);
            _canc(_isMin(current,key));
            _canc(current->getRight(),key);
...
public:
    ...
    void ins(T key)
    {
        ...
        {
            p=tmp;
            if(key.getCavalli() <= tmp->getKey().getCavalli()) tmp=tmp->getLeft();
        ...
        {
            newN->setParent(p);
            if(key.getCavalli() <= p->getKey().getCavalli())p->setLeft(newN);
